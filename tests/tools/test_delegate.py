...
class TestBlockedTools(unittest.TestCase):
    ...
    def test_constants(self):
        ...
        self.assertEqual(_MAX_SPAWN_DEPTH_CAP, 12)
...
class TestMaxSpawnDepth(unittest.TestCase):
    ...
    def test_max_spawn_depth_clamped_above_cap(self, mock_cfg):
        import logging
        from tools.delegate_tool import _get_max_spawn_depth
        with self.assertLogs("tools.delegate_tool", level=logging.WARNING) as cm:
            result = _get_max_spawn_depth()
        self.assertEqual(result, 12)
        self.assertTrue(any("clamping to 12" in m for m in cm.output))

    @patch("tools.delegate_tool._load_config",
           return_value={"max_spawn_depth": 5})
    def test_max_spawn_depth_allows_omo_depth_five(self, mock_cfg):
        from tools.delegate_tool import _get_max_spawn_depth
        self.assertEqual(_get_max_spawn_depth(), 5)
